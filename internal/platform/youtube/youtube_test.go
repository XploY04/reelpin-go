...
func TestAPublishedTranscriptSkipsTheMediaHalf(t *testing.T) {
	...
	input, ok := actor.lastInput.(map[string]any)
	if !ok || input["subtitlesLanguage"] != "any" || input["subtitlesFormat"] != "plaintext" || input["preferAutoGeneratedSubtitles"] != true {
		t.Fatalf("actor input = %#v, want plaintext auto-generated subtitles in any language", actor.lastInput)
	}
	if probe.calls != 0 {
...
func TestActorDescriptionIsUsedWhenSubtitlesAreMissing(t *testing.T) {
	actor := &fakeActor{items: mustItems(t, []map[string]any{{
		"id":           "abc123",
		"title":        "Making a chair from one board",
		"text":         "The video's own published description.",
		"thumbnailUrl": "https://i.ytimg.com/vi/abc123/maxres.jpg",
		"subtitles":    nil,
	}})}
	page := serve(t, func(http.ResponseWriter, *http.Request) {
...
func TestOEmbedKeepsAPublicVideoSaveable(t *testing.T) {
	server := serve(t, func(w http.ResponseWriter, r *http.Request) {
		if got := r.URL.Query().Get("url"); got != "https://youtu.be/abc123" {
			t.Errorf("oEmbed url = %q", got)
		}
...
func TestAnActorResultForAnotherVideoIsIgnored(t *testing.T) {
	...
	if prepared, ok := New(deps).prepareFromActor(
...
func (f *fakeActor) Run(_ context.Context, _ string, input any) ([]json.RawMessage, error) {
	f.calls++
	f.lastInput = input
