...
// related. A URL keeps the set portable across databases, where save ids are
// not.
...
func (s *Service) Coverage(ctx context.Context, userID string, set LabeledSet) (present int, total int, err error) {
	...
	err = s.pool.QueryRow(ctx, `
		...
		WHERE s.user_id = $1 AND c.normalized_url = ANY($2)`,
...
// The set is keyed by reel URL, so results are mapped back before scoring.
