...
func Run(ctx context.Context, command Command) (Result, error) {
	...
	// A child never inherits proxy settings: an egress proxy would carry a
	// download somewhere the address checks never saw.
	process.Env = scrubProxies(command.Env)
...
// scrubProxies removes every proxy variable, from the caller's env or the
// process env when none was given.
func scrubProxies(env []string) []string {
	if env == nil {
		env = os.Environ()
	}
