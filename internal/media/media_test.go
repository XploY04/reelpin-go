...
func TestDownloadRefusesAPathOutsideTheRunDirectory(t *testing.T) {
	...
	if _, err := NewYTDLP(runner).Download(context.Background(), "https://example.com/v", workDir, DownloadOptions{}); err == nil {
...
func TestDownloadClassifiesToolFailures(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := NewYTDLP(runner).Download(context.Background(), "https://example.com/v", t.TempDir(), DownloadOptions{})
...
func TestExtractAudioReportsAVideoWithNoSound(t *testing.T) {
	...
	if _, err := NewFFmpeg(runner).ExtractAudio(context.Background(), "video.mp4", t.TempDir()); !errors.Is(err, ErrNoAudioStream) {
...
func TestExtractAudioProducesMonoSpeechAudio(t *testing.T) {
	...
	path, err := NewFFmpeg(runner).ExtractAudio(context.Background(), "video.mp4", workDir)
	if err != nil {
...
func TestExtractAudioTreatsAnEmptyFileAsNoAudio(t *testing.T) {
	...
	if _, err := NewFFmpeg(runner).ExtractAudio(context.Background(), "video.mp4", workDir); !errors.Is(err, ErrNoAudioStream) {
		t.Fatalf("err = %v, want ErrNoAudioStream", err)
	}
}
