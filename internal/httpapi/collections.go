...
// Collections is everything the API needs from the collections implementation.
type Collections interface {
	List(ctx context.Context, userID string) ([]collections.Collection, error)
	Create(ctx context.Context, userID, name, description string, saveIDs []string) (collections.Collection, int, error)
	...
	Detail(ctx context.Context, collectionID, userID string, after *reels.Cursor, limit int) (collections.Detail, error)
	SharedByToken(ctx context.Context, token string, after *reels.Cursor, limit int) (collections.Shared, error)
	AddItems(ctx context.Context, collectionID, userID string, saveIDs []string) (int, error)
	RemoveItem(ctx context.Context, collectionID, userID, saveID string) error
	EnableLink(ctx context.Context, collectionID, userID string) (string, string, time.Time, error)
...
func (s *Server) writeCollectionError(w http.ResponseWriter, r *http.Request, err error, code, message string) {
	...
	case errors.Is(err, collections.ErrNotFound):
		notFoundError(w, "collection_not_found")
	...
	case errors.Is(err, collections.ErrInviteInvalid):
		writeError(w, http.StatusBadRequest, errorBody{
			Code:    "collection_invite_invalid",
			Message: "This invite is no longer valid.",
		})
	default:
		s.deps.Logger.Error(code, "route", r.Pattern, "error", err)
...
func (s *Server) handleCreateCollection(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(input.Name) == "" {
		validationError(w, "name", "a collection name is required")
		return
	}
...
func (s *Server) handleCollectionDetail(w http.ResponseWriter, r *http.Request) {
	...
	after, limit, ok := itemPageParams(w, r)
	if !ok {
		return
	}

	detail, err := s.deps.Collections.Detail(r.Context(), id.String(), requestUserID(r), after, limit)
	if err != nil {
...
// handleSharedCollection serves the one public-share route: the unguessable
// token in the path is the whole authorization.
func (s *Server) handleSharedCollection(w http.ResponseWriter, r *http.Request) {
	...
	shared, err := s.deps.Collections.SharedByToken(r.Context(), r.PathValue("token"), after, limit)
	if err != nil {
		s.writeCollectionError(w, r, err, "collection_detail_failed", "Could not load the collection right now.")
		return
	}
...
func (s *Server) handleAddCollectionItems(w http.ResponseWriter, r *http.Request) {
	...
	if len(input.ReelIDs) == 0 {
		validationError(w, "reel_ids", "at least one reel id is required")
		return
	}
...
func (s *Server) handleRemoveCollectionItem(w http.ResponseWriter, r *http.Request) {
	...
	saveID, ok := pathUUID(w, r, "reel_id", "reel_not_found")
	if !ok {
		return
	}

	if err := s.deps.Collections.RemoveItem(r.Context(), id.String(), requestUserID(r), saveID.String()); err != nil {
		s.writeCollectionError(w, r, err, "collection_items_failed", "Could not update the collection right now.")
		return
	}
	writeJSON(w, http.StatusOK, okResponse{OK: true})
...
func (s *Server) handleEnableCollectionLink(w http.ResponseWriter, r *http.Request) {
	...
	url, token, expiresAt, err := s.deps.Collections.EnableLink(r.Context(), id.String(), requestUserID(r))
	if err != nil {
	...
	writeJSON(w, http.StatusOK, collectionLinkResponse{
		URL: url, Token: token, ExpiresAt: expiresAt.UTC().Format(time.RFC3339Nano),
	})
}
...
func (s *Server) handleRemoveCollectionMember(w http.ResponseWriter, r *http.Request) {
	...
	memberID, ok := pathUUID(w, r, "member_user_id", "collection_not_found")
	if !ok {
		return
	}
...
func (s *Server) handleCreateCollectionInvite(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, collectionInviteResponse{
		URL: url, Token: token, Role: role, ExpiresAt: expiresAt.UTC().Format(time.RFC3339Nano),
	})
}
...
// itemPageParams are the cursor and limit every item page accepts, the same
// opaque-cursor rule the reel list uses.
func itemPageParams(w http.ResponseWriter, r *http.Request) (*reels.Cursor, int, bool) {
	query := r.URL.Query()
