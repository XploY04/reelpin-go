...
func TestNormalizeCategoryNeverLeavesEmptyLabels(t *testing.T) {
	got := NormalizeCategory(Category{Category: "  ", Subcategory: "", SecondaryCategories: []string{"", " Trips "}})
	if got.Category != "Other" || got.Subcategory != "Other" {
		t.Fatalf("category = %+v, want the Other fallback", got)
	}
	if len(got.SecondaryCategories) != 1 || got.SecondaryCategories[0] != "Trips" {
		t.Errorf("secondary = %v", got.SecondaryCategories)
	}
}

func TestJSONBodyTolerationOfCodeFences(t *testing.T) {
	...
	for _, tt := range tests {
		if got := jsonBody(tt.in); got != tt.want {
			t.Errorf("jsonBody(%q) = %q, want %q", tt.in, got, tt.want)
		}
	}
}
